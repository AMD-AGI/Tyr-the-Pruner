...
from src.common_utils import fix_seed, calculate_tensor_memory
from src.metric_utils import compute_perplexity, compute_kl_div, compute_sparse_kl_div, compute_perplexity_layer_per_layer
    ...
    while tokens_used < num_tokens:  # generate minibatch with exactly num_tokens tokens
        ...
        if tokens_used + calibration_data[minibatch_id].shape[1] > num_tokens:
            ...
            if fitness_fn == "kl":
                ...
                target_logits_minibatch.append((target_logits[minibatch_id][0][:, : num_tokens - tokens_used],
                                                target_logits[minibatch_id][1][:, : num_tokens - tokens_used] ))
...
def parse_args():
    ...
    # Model params
    parser.add_argument("--model_name_or_path", required=True, type=str, help="The name or path to the model being pruned")
    ...
    parser.add_argument("--max_total_deviation", default=99999, type=int, help="Max admissible total deviation (sum of absolute differences to uniform pruning).")
    # Logging params
    ...
    parser.add_argument("--lowmem_eval", action="store_true", help="Whether to use memory efficient evaluation.")
    parser.add_argument("--use_fast_tokenizer", action="store_true", help="Whether to use fast tokenizer.")
    
    # Save params
    parser.add_argument("--configuration_name", type=str, default="final_configuration.txt", help="Name of final configuration")
    parser.add_argument("--json_log_name", type=str, default="data.json", help="Name of json_log")
...
def main():
    ...
    # Load eval datasets
    args.eval_sequence_length = args.eval_sequence_length or min(model.config.max_position_embeddings, 4096)
    eval_datasets = []
    ...
    if args.fitness_fn == "kl":
        ...
        pbar = trange(0, len(calibration_data), desc="Computing target logits (calib)", leave=False)
        for i in pbar:
            with torch.no_grad():
                logits = model(calibration_data[i].to(device)).logits.cpu()
                target_logits.append(logits)
                total_memory_usage += calculate_tensor_memory(logits)
                pbar.set_description(f"Computing target logits (calib) - Memory: {total_memory_usage:.4f} GB")
        ...
        for i in pbar:
            with torch.no_grad():
                ...
                target_logits.append((topk_values, topk_indices.to(torch.int32)))
                total_memory_usage += calculate_tensor_memory(topk_values) + calculate_tensor_memory(topk_indices)
                pbar.set_description(f"Computing target logits (calib) - Memory: {total_memory_usage:.4f} GB")
    ...
    for generation in range(args.generations):
        ...
        if generation % args.eval_every == 0:
            for eval_dataset_name, eval_dataset in zip(args.eval_datasets, eval_datasets):
                if args.lowmem_eval:
                    ppl_eval = compute_perplexity_layer_per_layer(model, eval_dataset, offload=True)
                else:
                    ppl_eval = compute_perplexity(model, eval_dataset)
                print(f"{eval_dataset_name}: {ppl_eval:.2f}")
                log_dict["ppl_eval"].append(ppl_eval)
                best_wikitext2_ppl = min(ppl_eval, best_wikitext2_ppl)
        if args.log_wandb:
            wandb.log(log_dict)
        ...
        while len(offspring_list) < args.offspring:
            ...
            # mutate offspring
            num_flips = min(random.randint(1, 4), random.randint(1, 4))  # bias towards lower values
            for _ in range(num_flips):
                # positions where sparsity can be decreased    todo: all heads or all neurons
                target_id = -1
                while True:
                ...
                while True:
                    ...
                    if abs(level + 1) > args.max_level or target_id == -1 or (target_id + incr_id) % 2 != 0:
                        continue
                    if os.path.exists(os.path.join(args.sparse_weights_path, layer_name, f"{level + 1}.pth")):
                        break
                offspring[decr_id] -= 1
        ...
        print(f"One generation time: {end_time - start_time:.4e} s")
        log_dict["train_fitness"].append(train_fitness)
    ...
    for eval_dataset_name, eval_dataset in zip(args.eval_datasets, eval_datasets):
        if args.lowmem_eval:
            ...
            ppl_eval = compute_perplexity(model, eval_dataset)
        print(f"{eval_dataset_name}: {ppl_eval:.2f}")
        log_dict[f"ppl_eval"].append(ppl_eval)
    if args.lowmem_eval:
        ppl_train = compute_perplexity_layer_per_layer(model, calibration_data, offload=True)
    else:
        ppl_train = compute_perplexity(model, calibration_data)
    print(f"ppl_train: {ppl_train:.2f}")
    print(f"best wikitext2 ppl: {best_wikitext2_ppl:.2f}")
    log_dict["ppl_train"].append(ppl_train)
    if args.log_wandb:
        wandb.log(log_dict)
    with open(args.json_log_name, 'w') as f:

...
def get_intermediate_size(model: AutoModelForCausalLM):
    if model.config.model_type in ("llama", "gemma", "gemma2", "phi3", "opt", "mistral"):
        return model.config.intermediate_size
    else:
        raise ValueError(f"{model.config.model_type} is not supported.")
    
def get_head_size(model: AutoModelForCausalLM):
    if model.config.model_type in ("llama", "gemma", "gemma2", "phi3", "opt", "mistral"):
        return int(get_hidden_size(model) // model.config.num_attention_heads)
    else:
        raise ValueError(f"{model.config.model_type} is not supported.")
    
def get_o_proj_params(model: AutoModelForCausalLM):
    first_layer = get_layers(model)[0]
    return first_layer.self_attn.o_proj.weight.numel()

def get_down_proj_params(model: AutoModelForCausalLM):
    first_layer = get_layers(model)[0]
    return first_layer.mlp.down_proj.weight.numel()
